...
#define MAX_LEXER_SCRATCH_BUFFER_SIZE kilobytes(8)
...
fz_internal b32 token_is_trivia(Token* token);
fz_internal b32 token_is_comment(Token* token);

fz_internal s32    lexer_token_index(Lexer* lexer, s32 lookahead); /* Ensure we get a valid index into the circular buffer */
fz_internal Token* lexer_reserve_token_slot(Lexer* lexer); /* Add a new incoming token to buffer */
fz_internal void   lexer_dump_tokens(Lexer* lexer, String path, Trivia_Flags trivia_tokens, Emit_Structures emit_structures);

fz_internal b32    lexer_init_with_single_file_path(Lexer* lexer, String path, Trivia_Flags trivia_flags, Emit_Structures emit_structures); /* Attaches a file to the lexer for it to parse */
fz_internal b32    lexer_init_from_string(Lexer* lexer, String source, Trivia_Flags trivia_flags, Emit_Structures emit_structures); /* Initializes the lexer with a string */
fz_internal b32    lexer_init_from_string_with_arena(Lexer* lexer, Arena* arena, String source, Trivia_Flags trivia_flags, Emit_Structures emit_structures); /* Initializes the lexer with a string and a given arena by the caller */
fz_internal void   lexer_free(Lexer* lexer); /* Frees the lexer */
fz_internal Token* lexer_make_new_token(Lexer* lexer); /* Returns a new token */
fz_internal Token* lexer_make_token_from_next_n_characters(Lexer* lexer, Token* token, Token_Kind kind, u32 count); /* Returns the nth characters */
fz_internal Token* lexer_peek_token(Lexer* lexer); /* Creates and puts a new token into incoming tokens */
fz_internal Token* lexer_peek_nth_token(Lexer* lexer, s32 nth); /* Returns the nth token. Nth must be smaller than MAX_LOOKAHEAD_TOKENS */
fz_internal s16    lexer_peek_character(Lexer* lexer); /* Returns the next character without advancing the lexer */
fz_internal s16    lexer_peek_nth_character(Lexer* lexer, u32 nth); /* Returns the nth character without advancing the lexer. 0 is current character, 1 is next character, etc... */
fz_internal void   lexer_rewind_token(Lexer* lexer, u32 count); /* Used to undo a call to lexer_eat_token */
fz_internal void   lexer_eat_character(Lexer* lexer); /* Advances the lexer by 1 character */
fz_internal void   lexer_eat_token(Lexer* lexer); /* Advances lexer by 1 token */
fz_internal void   lexer_eat_trivia(Lexer* lexer); /* Advances over all spaces */
fz_internal Token* lexer_current_token(Lexer* lexer); /* Returns the current token. Doesn't peek, doesn't make. Can return garbage. */

// @Section: Parsing helpers
fz_internal void lexer_parse_single_character_token(Lexer* lexer, Token* token, Token_Kind kind); /* Parses a token that is a single character long */
fz_internal void lexer_parse_identifier(Lexer* lexer, Token* token); /* Parses the next token as an identifier */
fz_internal void lexer_parse_number(Lexer* lexer, Token* token); /* Parses the next token as a number */
fz_internal void lexer_parse_string_literal(Lexer* lexer, Token* token); /* Parses a string literal that is in between double quotes */
fz_internal void lexer_parse_string_backtick(Lexer* lexer, Token* token); /* Parses a backtick string that is in between backticks */
fz_internal void lexer_parse_character_literal(Lexer* lexer, Token* token); /* Parses a character literal that is in between single ticks */
fz_internal void lexer_parse_trivia(Lexer* lexer, Token* token); 
fz_internal void lexer_parse_line_comment(Lexer* lexer, Token* token); /* Parses a line comment defined by double slash */
fz_internal void lexer_parse_block_comment(Lexer* lexer, Token* token); /* Parses a block comment defined by slash asterisk up to asterisk slash */
fz_internal Token_Kind lexer_classify_trivia(u8 c);
fz_internal b32        lexer_should_emit_trivia(Lexer* lexer, Token_Kind kind);
